...
def make_company(
    ticker: str = "WEGE3", name: str = "WEG S.A.", sector: str = "Industrial"
):
...
class TestDetectAlerts:
    ...
    def test_multiple_alerts_at_once(self) -> None:
        ...
        assert AlertType.COMMODITY_HEAVY in alert_types
        assert (
            AlertType.OVER_CONCENTRATED not in alert_types
        )  # need at least 2 for over-concentrated
    ...
    def test_tesouro_zero_message_is_humorous(self) -> None:
        ...
        assert (
            "corajoso" in tesouro_alerts[0].message.lower()
            or "paraquedas" in tesouro_alerts[0].message.lower()
        )
...
class TestPortfolioServiceAnalyze:
    ...
    async def test_analyze_returns_correct_shape_with_mocked_ai(self) -> None:
        ...
        service = self._make_service(
            rag_service=mock_rag, analysis_service=mock_analysis
        )
    ...
    async def test_analyze_raises_ticker_not_found_for_unknown_tesouro_ticker(
        self,
    ) -> None:
    ...
    async def test_analyze_degrades_when_company_exists_but_has_no_financials(
        self,
    ) -> None:
